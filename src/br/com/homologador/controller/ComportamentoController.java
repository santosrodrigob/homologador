...
import br.com.homologador.services.impl.ComportamentoServicesImpl;
...
public class ComportamentoController implements Acao {
	...
	public String listarAction(HttpServletRequest request, HttpServletResponse response, Connection connection)
		...
		if(StringUtils.isLong(codigoModuloParameter)) {
			comportamento.setCodigoModulo(Integer.valueOf(codigoModuloParameter));
		}

		if(StringUtils.isLong(codigoFeatureParameter)) {
			comportamento.setCodigoFeature(Integer.valueOf(codigoFeatureParameter));
		}

		if(StringUtils.isLong(codigoTipoParameter)) {
			comportamento.setCodigoTipo(Integer.valueOf(codigoTipoParameter));
		}

		if(StringUtils.isLong(codigoComportamentoParameter)) {
			comportamento.setCodigoComportamento(Integer.valueOf(codigoComportamentoParameter));
		}

		if(StringUtils.isLong(inativoParameter)) {
			comportamento.setInativo(Integer.valueOf(inativoParameter));
		}

		ComportamentoServices comportamentoServices = new ComportamentoServicesImpl(connection);
		try 
		{
			comportamentoServices.getAll(comportamento);
		} 
		catch (Exception e) 
		{
			e.printStackTrace();
		...
		List<Comportamento> listaComportamentos = comportamento.getListaComportamentos();

		listaComportamentos.sort(Comparator.comparing(c -> c.getDescricaoModulo()));
		
		request.setAttribute(ConstantDataManager.OBJETO_LISTA_COMPORTAMENTOS, listaComportamentos);
		
		return "forward:comportamento/lista-comportamento.jsp";
	}
	...
	public String adicionarAction(HttpServletRequest request, HttpServletResponse response, Connection connection)
		...
		String codigoComportamentoParameter = request.getParameter(ConstantDataManager.PARAMETER_CODIGO_COMPORTAMENTO);
		...
		String descricaoComportamentoParameter = request.getParameter(ConstantDataManager.PARAMETER_DESCRICAO_COMPORTAMENTO);
		...
		if(StringUtils.isLong(codigoModuloParameter) && StringUtils.isLong(codigoFeatureParameter) && StringUtils.isLong(codigoTipoParameter)
				&& !StringUtils.isNull(codigoTipoParameter) && StringUtils.isLong(testeFeatureParameter)) {
			comportamento.setCodigoModulo(Integer.valueOf(codigoModuloParameter));
			comportamento.setCodigoFeature(Integer.valueOf(codigoFeatureParameter));
			comportamento.setCodigoTipo(Integer.valueOf(codigoTipoParameter));
			comportamento.setTesteFeature(Integer.valueOf(testeFeatureParameter));
			comportamento.setDescricaoComportamento(descricaoComportamentoParameter);
		} else {
		...
		try 
		{
			isUpdated = comportamentoServices.adicionar(comportamento);
		} 
		catch (Exception e) 
		{
			e.printStackTrace();
		...
		return "redirect:/homologador/entrada?controller=Comportamento&acao=Listar";	
	}
	...
	public String mostrarAction(HttpServletRequest request, HttpServletResponse response, Connection connection)
		...
		String codigoComportamentoParameter = request.getParameter(ConstantDataManager.PARAMETER_CODIGO);
		Integer codigo = null;

		if(StringUtils.isLong(codigoComportamentoParameter)) {
			codigo = Integer.valueOf(codigoComportamentoParameter);
		} else {
			message = ConstantDataManager.MESSAGE_DADOS_OBRIGATORIOS_NAO_INFORMADO;
		}
		...
		try 
		{
			comportamento = comportamentoServices.getComportamentoById(codigo);
		...
		request.setAttribute(ConstantDataManager.MESSAGE, message);
		request.setAttribute(ConstantDataManager.OBJETO_COMPORTAMENTO, comportamento);
		
		return "forward:comportamento/form-comportamento-editar.jsp";
	}
	...
	public String removerAction(HttpServletRequest request, HttpServletResponse response, Connection connection)
		...
		try 
		{
			isRemoved = comportamentoServices.remove(codigo);
		}
		catch (Exception e) 
		{
			e.printStackTrace();
	...
	public String openAction(HttpServletRequest request, HttpServletResponse response, Connection connection)
		...
		if(StringUtils.isNull(filtrarParameter)) {
			return "forward:comportamento/form-comportamento-novo.jsp";
		} else {
			return "forward:comportamento/filtrar-comportamento.jsp";
		}
	}
}
